...
def getAppData(kernels, x, y, xaxis_title, correlmap):
    ...
    return (
        ...
        num_over,
        num_under,
        num_less_than_one_percent,
        aggregate_err,
        ret_rpd,
        num_less_than_ten_percent,
        rmse,
    )
...
def getCorrelCsvRaw(anno, names, x, y):
    ...
    for k in names:
        out_csv += "{0},{1:.2f},{2:.2f},{3:.2f}\n".format(
            k, x[count], y[count], y[count] / (x[count] + 0.000001)
        )
        count += 1
    return out_csv
...
def make_pretty_app_list(apps_included):
    ...
    for app in app_list:
        if len(apps_included[app]) > 0:
            ...
            ret_str += "{0} :: {1} :: {2:.2f}%\n".format(
                app, len(apps_included[app]), avg_err
            )
        else:
...
def make_submission_quality_image(image_type, traces, hw_cfg):
    ...
    for (
        ...
        print_anno += (
            anno
            + " :: {0} high error points dropped from Err calc. {1} dropped for HW too low (>{2})\n".format(
        ...
        kernel_csv_file_contents += "{0}\n\n".format(
            getCorrelCsvRaw(anno, trace.text, trace.x, trace.y)
        )
        ...
        app_csv_file_contents += "{0}\n\n".format(
            getCorrelCsvRaw(anno, apps, appx, appy)
        )
        kernel_data.append(trace)

        app_anno = (
            cfg
            + " ({0} apps ({5} < 1% Err, {3} under, {4} over, {8} < 10% Err)) [Correl={1:.4} Err={2:.2f}% Agg_Err={6:.2f}% RPD={7:.2f}%,NMSE={9:.2f}]".format(
                len(apps),
                correl_co,
                avg_err,
                num_under,
        ...
        if (
            "Cycles" in layout.xaxis.title.text
            or "IPC" in layout.xaxis.title.text
            or "Instructions" in layout.xaxis.title.text
        ):
            name_text = (
                "<b>"
                + trace.name
                + " [Correl={0:.2} MAE={1:.0f}%]</b>".format(correl_co, avg_err, nmse)
            )
        else:
            name_text = (
                "<b>"
                + trace.name
                + " [Correl={0:.2} NRMSE={1:.2f}]</b>".format(correl_co, nmse)
            )
    ...
    print("Plotting {0} : [{1}]\n{2}".format(hw_cfg, layout.title.text, print_anno))
    TEXT_SIZE = 25
    ...
    png_layout.legend = dict(
        ...
        y=1.2,
        traceorder="normal",
        font=dict(family="sans-serif", size=10, color="#000"),
        bgcolor="#E2E2E2",
        bordercolor="#FFFFFF",
        borderwidth=2,
    )
    xyline = go.Scatter(
        x=[layout.xaxis.range[0] + 1, layout.xaxis.range[1]],
        y=[layout.xaxis.range[0] + 1, layout.xaxis.range[1]],
        showlegend=False,
        mode="lines",
    )
    ...
    if "linear" == correlmap.plottype:
        app_min = -5
        legypos = float(options.legend) * 0.95
        legxpos = -0.25
        axisrange = [app_min * 0.95, app_max * 1.05]
    else:
    ...
    app_layout = Layout(
        ...
        legend=dict(
            ...
            y=legypos,
            traceorder="normal",
            font=dict(family="sans-serif", size=15, color="#000"),
    ...
    if image_type != "":
        ...
        Figure(data=app_data, layout=app_layout).write_image(
                    ...
                    for item in row[1:]:
                        ...
                        if len(split) > 1:
                            ...
                            if appargs == last_appargs:
                                ...
                                num = 0
                            logger.log(
                                "Found appargs {0}. Kernel {1}".format(appargs, kname)
                            )
            ...
            if state == "process-cfgs":
                ...
                for x in row[1:]:
                    if count < len(klist):
                        ...
                        print(
                            "More row entries than kernels processed. Likely because of an average column",
                            file=sys.stderr,
                        )
                        break
                    try:
...
def parse_hw_csv_2(csv_file, hw_data, appargs, kdata, logger):
    ...
    if cfg != "" and cfg != None:
        cfg = re.sub(
            r" \(\d+\)$", "", cfg
        )  # Strip off the (0), (1), etc that some profiler versions put on the end of the device name
        ...
        for row in reader:
            ...
            if state == "kernel_proc":
                ...
                if cfg != "" and cfg != row[cfg_col]:
                    print(
                        "data for more than one device in {0}..{1}:{2}".format(
                            csv_file, cfg, elem
                        )
                    )
                    exit()
...
def summarize_hw_data(hw_data, logger):
    ...
    for device, appargslist in hw_data.items():
        ...
        print(
            "HW Summary for {0} [Contains {1} Apps]:".format(device, len(appargslist))
        )
    print("----------------------------------------------------------------\n\n")
    ...
    for device, appargslist in hw_data.items():
        ...
        for appargs, kdata in appargslist.items():
            ...
            if "Name" in kdata[0]:
                logger.logchan(
                    "\t\t\tSample Kernel 0 [{0}]:".format(kdata[0]["Name"][0][0:64]),
                    "hwsummary",
                )
            elif "Kernel Name" in kdata[0]:
                logger.logchan(
                    "\t\t\tSample Kernel 0 [{0}]:".format(
                        kdata[0]["Kernel Name"][0][0:64]
                    ),
                    "hwsummary",
                )
            ...
            for stat in sample_stats:
                if stat in kdata[0]:
                    logger.logchan(
                        "\t\t\t\t{0}: Numsamples={1} [min={2},max={3}]".format(
                            stat,
                            len(kdata[0][stat]),
                            min(kdata[0][stat]),
                            max(kdata[0][stat]),
                    ...
                    nothere.add(stat)
            logger.logchan(
                "\t\t\t\t{0} not gathered in this kernel".format(nothere), "hwsummary"
            )
        logger.logchan(
            "-----------------------------------------------------------------\n\n",
            "hwsummary",
...
parser.add_option(
    "-r", "--rename_data", dest="rename_data", default="", help="Rename the data series"
)
parser.add_option(
    "-m",
    "--marker_order",
...
benchmarks = []
options.hardware_dir = common.dir_option_test(
    options.hardware_dir, "../../hw_run/", this_directory
)
options.data_mappings = common.file_option_test(
    options.data_mappings, "correl_mappings.py", this_directory
)
...
for cfg, sim_for_cfg in sim_data.items():
    ...
    for correl in correl_list:
        if correl.hw_name != "all" and correl.hw_name not in hw_cfg:
            logger.log(
                "for cfg:{0}, hw_cfg:{1} - Skipping plot:\n{2}".format(
                    cfg, hw_cfg, correl
                )
            )
            continue
        ...
        for appargs, sim_klist in sim_for_cfg.items():
            if appargs in hw_data[hw_cfg]:
                ...
                if len(sim_klist) <= len(hw_klist):
                    logger.log(
                        "Found hw/sim match for {0}. Sim={1}. HW={2}".format(
                            appargs, len(sim_klist), len(hw_klist)
                        )
                    )
                    ...
                    for sim in sim_klist:
                        ...
                        try:
                            ...
                            e = sys.exc_info()[0]
                            logger.log(
                                "Potentially uncollected stat in {0}.Error: {1}".format(
                                    correl.hw_eval, e
                                )
                            )
                            #                            print hw
                        ...
                        try:
                            ...
                            logger.log(
                                "Division by zerofor  stat in {0}.Error: {1}".format(
                                    correl.sim_eval, e
                                )
                            )
                            count += 1
                        ...
                        label_array.append(
                            (appargs + "--" + sim_klist[count]["Kernel"])
                            + " (Err={0:.2f}%,HW-Range=+{1:.2f}%/-{2:.2f}%)".format(
                                err, hw_high, hw_low
                            )
                        )
                        count += 1
                    ...
                    logger.log(
                        "For appargs={0}, HW/SW kernels do not match HW={1}, SIM={2} and software has more than hardware\n".format(
                            appargs, len(hw_klist), len(sim_klist)
                        )
                    )
        ...
        logger.log(
            "Sim apps no HW:\n{0}\nHW apps no sim data:\n{1}".format(
                sim_appargs_leftover, hw_appargs_leftover
            )
        )
        ...
        if not options.err_off:
            anno = (
                cfg
                + " ({0} apps, {1} kernels ({6} < 1% Err, {4} under, {5} over)) [Correl={2:.4} Err={3:.2f}%]".format(
                    appcount,
                    kernelcount,
                    correl_co,
                    avg_err,
            ...
            anno = (
                cfg
                + " ({0} apps, {1} kernels ({6} < 1% Err, {4} under, {5} over)) [Correl={2:.4}]".format(
                    appcount,
                    kernelcount,
                    correl_co,
                    avg_err,
        ...
        fig_data[(correl.plotfile, hw_cfg)].append(
            (
                trace,
                layout,
                cfg,
